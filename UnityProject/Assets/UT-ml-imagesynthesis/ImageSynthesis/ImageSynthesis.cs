...
public class ImageSynthesis : MonoBehaviour {
	...
	{
		int targetDisplay = 2; // start from display 3
	...
	public byte[] GetPNGBufferFromCapturePass(string capturePassName, int width, int height)
	{
		foreach (var capturePass in capturePasses)
        {
			if (capturePass.name == capturePassName)
			{
				return GetPNGBuffer(capturePass.camera, width, height, capturePass.supportsAntialiasing, capturePass.needsRescale);
            }
        }
		
		Debug.LogError("cant get PNG buffer of " + capturePassName + ", capture pass not found");
		return null;
	}

	private byte[] GetPNGBuffer(Camera cam, int width, int height, bool supportsAntialiasing, bool needsRescale)
	{
		var mainCamera = GetComponent<Camera>();
		var depth = 24;
